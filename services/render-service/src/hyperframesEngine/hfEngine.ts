...
export interface HfRenderInput {
  ...
  /** Optional full project directory contents: relative path -> file content. */
...
export async function renderHf(
  ...
  try {
    ...
    // If a full project directory is supplied, write it out first. The inline
    // compositionHtml (if any) wins for the entry file so the existing API still
    // works, but projectFiles lets callers ship sub-compositions, components, etc.
    ...
    // Only write index.html from inline HTML when no projectFiles already wrote it,
    // or when inline HTML is explicitly provided and non-empty.
